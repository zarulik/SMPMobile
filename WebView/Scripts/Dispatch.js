...
class DispatchPage {
  ...
  CalculateDispatchDetailPrice(dispatchLine){
    ...
    if(item.SaleMethod=="P")
    {
      ...
      salePrice=parseFloat((item.StandardUnitPrice-item.StandardUnitPrice*item.SalesPromotionDiscountPercent/100).RoundTo(pageModel.ciOptions.NumberOfDecimalPlacesInPrice));
    }
    ...
    price = parseFloat(price.RoundTo(pageModel.ciOptions.NumberOfDecimalPlacesInPrice)) * item.SalesUMConvFctr;
        ...
        if(elem.ExpirationDate>pageModel.taskInfo.TaskDate){
          switch(elem.PricingMethod)
          {
            ...
            case "D":
              ...
              break;
            case "O":
              contractPrice = ( elem.OverridePrice / elem.UnitOfMeasureConvFactor ) * 1;//TODO MODIFY THIS LINES WHEN UOM WILL BE AVAILABLE IN MOBILE
              break
            case "P":
              contractPrice = calculatedPrice - ( elem.PriceOff / elem.UnitOfMeasureConvFactor ) * 1;//TODO MODIFY THIS LINES WHEN UOM WILL BE AVAILABLE IN MOBILE
              break;
          }
        }
      });
  ...
  UseBillToCustomer(){
    ...
    for (var i = 0; i < pageModel.arBillToSoldTos.length; i++) {
      if (pageModel.arBillToSoldTos[i]["SoldToDivisionNo"] == pageModel.taskInfo.ARDivisionNo && pageModel.arBillToSoldTos[i]["SoldToCustomerNo"] == pageModel.taskInfo.CustomerNo) {
  ...
  InitializeDispatchEvents(
    ...
    $('#newDetailInfo #quantityInput').change(function(e){
      ...
      $('#newDetailInfo #unitPriceInput').val(newPrice.RoundTo(UnitPriceDecimals));
    ...
    $('#newDetailInfo #unitPriceInput').change(function(e){
      $('#newDetailInfo #extensionInput').val(parseFloat($(this).val()*$('#newDetailInfo #quantityInput').val()).RoundTo(2));
    });
    ...
    $('#submitNewDetailButton').click(function(e){
      ...
      var newLine = {
        ...
        QuantityOrdered : parseFloat(parseFloat($('#newDetailInfo #quantityInput').val()).RoundTo(QuantityDecimals)),
        UnitPrice :  parseFloat(parseFloat($('#newDetailInfo #unitPriceInput').val()).RoundTo(UnitPriceDecimals)),
        ExtensionAmt : parseFloat(parseFloat($('#newDetailInfo #extensionInput').val()).RoundTo(2)),
      };
      ...
      smpMobileApi.InsertDispatchDetail(pageModel.dispatch.TaskNo,pageModel.dispatch.DispatchNo,newLine,priceCalculated,
        function(res) {
          ...
          function(savedLine) {
            smpMobileApi.GetDispatch(pageModel.dispatch.TaskNo,pageModel.dispatch.DispatchNo,
    ...
    $('#newDetailLaborInfo #hoursSpentInput').change(function(e){
      ...
      $('#newDetailLaborInfo #hoursSpentInput').val(validatedHoursSpent.RoundTo(QuantityDecimals));
      ...
      $('#newDetailLaborInfo #billintRateInput').val(calculatedBillingRate.RoundTo(UnitPriceDecimals));
    ...
    $('#newDetailLaborInfo #billintRateInput').change(function(e){
      $('#newDetailLaborInfo #laborExtensionInput').val(parseFloat($(this).val()*$('#newDetailLaborInfo #hoursSpentInput').val()).RoundTo(2));
    });
    ...
    $('#submitNewDetailLaborButton').click(function(e){
      ...
      var newLaborLine = {
        ...
        HoursSpent : parseFloat(parseFloat($('#newDetailLaborInfo #hoursSpentInput').val()).RoundTo(QuantityDecimals)),
        BillingRate :  parseFloat(parseFloat($('#newDetailLaborInfo #billintRateInput').val()).RoundTo(UnitPriceDecimals)),
        ExtensionAmt : parseFloat(parseFloat($('#newDetailLaborInfo #laborExtensionInput').val()).RoundTo(2)),
    ...
    $('#printDispatchButton').click(function(e){
      ...
      smpMobileApi.GenerateInvoice(pageModel.dispatch.TaskNo,pageModel.dispatch.DispatchNo,function(fileName){
        smpMobileApi.GetTaskAttachment(pageModel.dispatch.TaskNo,fileName,function(res){
          $('.loading').toggleClass('d-none');
          $('.loading div').toggleClass('loadingIcon');
          window.open(GetCacheValue(CacheIds.serverEndpoint)+'/api/dispatch/task/attachments/get/'+pageModel.dispatch.TaskNo+'/'+fileName+'?tokenID='+encodeURIComponent(res.Hash),'_blank');
        },
        function(err){
          $('.loading').toggleClass('d-none');
          $('.loading div').toggleClass('loadingIcon');
  ...
  InitializeEditDeleteEvents(){
    ...
    $('.editDetail').unbind().click(function(e){
      ...
      if(targetLineKey)
      {
        ...
        if(dispatchLine)
        {
          ...
          $('#editDetailInfo #quantityEditInput').val(dispatchLine.QuantityOrdered.RoundTo(QuantityDecimals));
          //$('#editDetailInfo #quantityEditInput').trigger("change");
          $('#editDetailInfo #quantityEditInput').attr('disabled',false);
          $('#editDetailInfo #unitPriceEditInput').val(dispatchLine.UnitPrice.RoundTo(UnitPriceDecimals));
    ...
    $('#editDetailInfo #quantityEditInput').unbind().change(function(e){
      ...
      $('#editDetailInfo #unitPriceEditInput').val(newPrice.RoundTo(UnitPriceDecimals));
      $('#editDetailInfo #unitPriceEditInput').trigger("change");
    });
    $('#editDetailInfo #unitPriceEditInput').unbind().change(function(e){
      var targetLineKey =  $('#submitEditDetailButton').attr('data-target').split('_')[1];
      var newExtension = $(this).val()*$('#editDetailInfo #quantityEditInput').val();
      var dispatchLine = FindRowByFieldNameValuePair(pageModel.dispatchDetail,'LineKey',targetLineKey);
      if(pageModel.soOptions.AllowDiscountRate && dispatchLine.LineDiscountPercent)
      {
        newExtension = newExtension * (( 100 - dispatchLine.LineDiscountPercent)/100);
      }
      $('#editDetailInfo #extensionEditInput').val(parseFloat(newExtension).RoundTo(2));
    });
    $('#submitEditDetailButton').unbind().click(function(e){
      ...
      if(targetLineKey)
      {
        ...
        if(modifyingLine && modifyingLineIndex!=null)
        {
          ...
          modifyingLine.QuantityOrdered = parseFloat(parseFloat($('#editDetailInfo #quantityEditInput').val()).RoundTo(QuantityDecimals));
          modifyingLine.UnitPrice = parseFloat(parseFloat($('#editDetailInfo #unitPriceEditInput').val()).RoundTo(UnitPriceDecimals));
          modifyingLine.ExtensionAmt = parseFloat(parseFloat($('#editDetailInfo #extensionEditInput').val()).RoundTo(2));
          ...
          smpMobileApi.UpdateDispatchDetail(
            ...
            modifyingLine,false,function(res) {
    ...
    $('.editDetailLabor').unbind().click(function(e){
      ...
      if(targetLineKey)
      {
        ...
        if(laborLine)
        {
          ...
          $('#editDetailLaborInfo #hoursSpentEditInput').val(laborLine.HoursSpent.RoundTo(QuantityDecimals));
          $('#editDetailLaborInfo #hoursSpentEditInput').trigger("change");
          $('#editDetailLaborInfo #hoursSpentEditInput').attr('disabled',false);
          $('#editDetailLaborInfo #billingRateEditInput').val(laborLine.BillingRate.RoundTo(UnitPriceDecimals));
    ...
    $('#editDetailLaborInfo #hoursSpentEditInput').unbind().change(function(e){
      var validatedHoursSpent = ds.ValidateHoursSpent(parseFloat($(this).val()));
      $('#editDetailLaborInfo #hoursSpentEditInput').val(validatedHoursSpent.RoundTo(QuantityDecimals));
      var laborLineNbr = $('#submitEditDetailLaborButton').attr('data-target').split("_")[1];
      var laborLine = FindRecordsByParams(pageModel.dispatchLabor,true,[{Name: "LineKey",Value: laborLineNbr}])[0];
      var overTimeStartDate = laborLine.OverTimeStartDate;
      var overTimeStartTime = laborLine.OverTimeStartTime;
      var calculatedBillingRate = ds.CalculateDispatchDetailLaborPrice({LaborSkillCode: $('#editDetailLaborInfo #laborSkillEditInput').val(), HoursSpent: validatedHoursSpent, OverTimeStartDate: overTimeStartDate, OverTimeStartTime : overTimeStartTime});
      $('#editDetailLaborInfo #billingRateEditInput').val(calculatedBillingRate.RoundTo(UnitPriceDecimals));
      $('#editDetailLaborInfo #billingRateEditInput').trigger("change");
    });
    $('#editDetailLaborInfo #billingRateEditInput').unbind().change(function(e){
      var targetLineKey =  $('#submitEditDetailLaborButton').attr('data-target').split('_')[1];
      var newExtension = $(this).val()*$('#editDetailLaborInfo #hoursSpentEditInput').val();
      var laborLine = FindRowByFieldNameValuePair(pageModel.dispatchLabor,'LineKey',targetLineKey);
      if(pageModel.soOptions.AllowDiscountRate && laborLine.LineDiscountPercent)
      {
        newExtension = newExtension * (( 100 - laborLine.LineDiscountPercent)/100);
      }
      $('#editDetailLaborInfo #laborExtensionEditInput').val(parseFloat(newExtension).RoundTo(2));
    });
    $('#submitEditDetailLaborButton').unbind().click(function(e){
      ...
      if(targetLineKey)
      {
        ...
        if(modifyingLine && modifyingLineIndex!=null)
        {
         ...
         modifyingLine.HoursSpent = parseFloat(parseFloat($('#editDetailLaborInfo #hoursSpentEditInput').val()).RoundTo(QuantityDecimals));
          modifyingLine.BillingRate = parseFloat(parseFloat($('#editDetailLaborInfo #billingRateEditInput').val()).RoundTo(UnitPriceDecimals));
          modifyingLine.ExtensionAmt = parseFloat(parseFloat($('#editDetailLaborInfo #laborExtensionEditInput').val()).RoundTo(2));
          modifyingLine.LaborCommentText = $('#editDetailLaborInfo #laborCommentEditInput').val();
          smpMobileApi.UpdateDispatchDetailLabor(
            pageModel.dispatch.TaskNo,
            pageModel.dispatch.DispatchNo,
            modifyingLine,function(res){
            ...
            function(err){
              ...
              smpMobileApi.ShowModal(
                "Issue",
                "Failed to get updated <strong>Dispatch Detail Labor</strong><br />"+err.Message,
                "danger",
                15000
              );
            });
            ...
            smpMobileApi.ShowModal(
              "Issue",
              "Failed to update <strong>Dispatch Detail Labor</strong><br />"+err.Message,
              "danger",
              15000
            );
          });
  ...
  FillDispatchAttachments(attachments) {
    ...
    attachments.forEach(attachment => {
      ...
      lineNumber++;
      $tr.append(
        '<td><div class="single-line"><a href="#" class="openAttachment" data-target="'+attachment.Name+':'+attachment.Hash+'">' +
          attachment.Name +
          "</a></div></td>"
      );
    ...
    $('.openAttachment').click(function(e){
      ...
      var url = GetCacheValue(CacheIds.serverEndpoint)+'/api/dispatch/task/attachments/get/'+pageModel.taskInfo.TaskNo+'/'+attachment.split(':')[0]+'?tokenID='+encodeURIComponent(attachment.split(':')[1]);
  ...
  FillPrintingModal(priceDecimal,qtyDecimal){
    ...
    if(pageModel.dispatchDetail.length>0)
    {
      ...
      dispatchDetailsString +='<div class="row boldHeader"><div class="col-4">Item Code</div><div class="col-2">Qty.</div><div class="col-2 amountField">Price</div><div class="col-4 amountField">Extension</div><div class="col-12">Description</div></div>';
      pageModel.dispatchDetail.forEach(dispatchDetailLine => {
        dispatchDetailsString += '<div class="row"><div class="col-4">'+(dispatchDetailLine.ItemType!="4"?PopulateLineFeeds(dispatchDetailLine.ItemCode):'')+'</div>'+'<div class="col-2">'+((dispatchDetailLine.ItemType!="4" && dispatchDetailLine.ItemType!="3")?dispatchDetailLine.QuantityOrdered.RoundTo(qtyDecimal):'')+'</div>'+
        '<div class="col-3 amountField">'+((dispatchDetailLine.ItemType!="4" && dispatchDetailLine.ItemType!="3")?dispatchDetailLine.UnitPrice.RoundTo(priceDecimal):'')+'</div>'+'<div class="col-3 amountField">'+(dispatchDetailLine.ItemType!="4"?dispatchDetailLine.ExtensionAmt.RoundTo(2):'')+'</div>'+
        '<div class="col-12">'+(dispatchDetailLine.ItemType!="4"?(dispatchDetailLine.ExtendedDesc?dispatchDetailLine.ExtendedDesc: dispatchDetailLine.ItemCodeDesc):'')+'</div>'+
        '<div class="col-12">'+(dispatchDetailLine.CommentText!=""?dispatchDetailLine.CommentText:'')+'</div></div>';
      });
    ...
    if(pageModel.dispatchLabor.length>0)
    {
      ...
      dispatchLaborsString +='<div class="row boldHeader"><div class="col-4">Labor Code</div><div class="col-2">Hours Spent</div><div class="col-2 amountField">Billing Rate</div><div class="col-4 amountField">Extension</div><div class="col-12">Description</div></div>';
      pageModel.dispatchLabor.forEach(dispatchDetailLaborLine => {
        dispatchLaborsString += '<div class="row"><div class="col-4">'+PopulateLineFeeds(dispatchDetailLaborLine.LaborSkillCode)+'</div>'+'<div class="col-2">'+dispatchDetailLaborLine.HoursSpent.RoundTo(qtyDecimal)+'</div>'+
        '<div class="col-3 amountField">'+dispatchDetailLaborLine.BillingRate.RoundTo(priceDecimal)+'</div>'+'<div class="col-3 amountField">'+dispatchDetailLaborLine.ExtensionAmt.RoundTo(2)+'</div>'+
        '<div class="col-12">'+(dispatchDetailLaborLine.LineDescExtended?dispatchDetailLaborLine.LineDescExtended : dispatchDetailLaborLine.LineDesc)+'</div>'+
        '<div class="col-12">'+(dispatchDetailLaborLine.LaborCommentText!=""?dispatchDetailLaborLine.LaborCommentText:'')+'</div></div>';
      });
      dispatchLaborsString+='</div>';
    }
    var dispatchTotalsString = '<br /><div class="col-12 text-right amountField">';
    dispatchTotalsString += '<p> Taxable Amount : '+pageModel.dispatch.TaxableAmt.RoundTo(2)+'</p>' 
                           +'<p> NonTaxable Amount : '+pageModel.dispatch.NonTaxableAmt.RoundTo(2)+'</p>' 
                           +'<p> Less Discount : '+pageModel.dispatch.DiscountAmt.RoundTo(2)+'</p>' 
                           +'<p> Freight : '+pageModel.dispatch.FreightAmt.RoundTo(2)+'</p>' 
                           +'<p> Sales Tax : '+pageModel.dispatch.SalesTaxAmt.RoundTo(2)+'</p>' 
                           +'<h5> Dispatch Total : '+pageModel.dispatch.DispatchTotal.RoundTo(2)+'</h5>'
    + (pageModel.dispatch.DepositAmt>0?
                           ('<h5> Deposit Amount : '+pageModel.dispatch.DepositAmt.RoundTo(2)+'</h5>'+
                            '<h5> Net Dispatch : '+(pageModel.dispatch.DispatchTotal-pageModel.dispatch.DepositAmt).RoundTo(2)+'</h5>'):'');
  ...
  UpdateDispatchDetailView(line,quantityDec,priceDec)
  {
    ...
    $('#dispatchDetailsTableBody #line_'+line.LineKey+' td:nth-child(3) div').text(line.QuantityOrdered.RoundTo(quantityDec));
    $('#dispatchDetailsTableBody #line_'+line.LineKey+' td:nth-child(4) div').text(line.UnitPrice.RoundTo(priceDec));
    $('#dispatchDetailsTableBody #line_'+line.LineKey+' td:nth-child(5) div').text(line.ExtensionAmt.RoundTo(2));
  ...
  UpdateDispatchDetailLaborView(labor,quantityDec,priceDec)
  {
    ...
    $('#dispatchDetailLaborsTableBody #labor_'+labor.LineKey+' td:nth-child(3) div').text(labor.HoursSpent.RoundTo(quantityDec));
    $('#dispatchDetailLaborsTableBody #labor_'+labor.LineKey+' td:nth-child(4) div').text(labor.BillingRate.RoundTo(priceDec));
    $('#dispatchDetailLaborsTableBody #labor_'+labor.LineKey+' td:nth-child(5) div').text(labor.ExtensionAmt.RoundTo(2));
